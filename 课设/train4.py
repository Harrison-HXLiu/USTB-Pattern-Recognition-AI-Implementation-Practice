...
if __name__ == "__main__":
    ...
    # ---------------------------------------------------------
    # 4. 论文绘图 (Paper Plots) - 更新版 (含对比模型)
    # ---------------------------------------------------------
    print("正在生成 Figure 1 和 Figure 2 (含MLP/StdSCN对比)...")
    ...
    # Subplot (a): 预测值对比
    plt.subplot(1, 2, 1)
    subset = 80 # 为了看清，只画前80个点
    plt.plot(y_test_real[:subset], 'k-', label='Actual', linewidth=2, alpha=0.8)
    plt.plot(y_pred_mlp_real[:subset], 'g:', label='MLP', linewidth=1.5, alpha=0.7)
    plt.plot(y_pred_std_real[:subset], 'b--', label='Std SCN', linewidth=1.5, alpha=0.7)
    plt.plot(y_pred_dwmp_real[:subset], 'r-', label='DWMP-SCN', linewidth=2)
    plt.xlabel('Sample Index')
    plt.ylabel('Power Output (MW)')
    plt.title('(a) Prediction Comparison')
    plt.legend(loc='upper right', frameon=True)

    # Subplot (b): 误差分布对比 (Histogram/KDE)
    plt.subplot(1, 2, 2)
    ...
    # 使用 Step 类型直方图，避免遮挡
    plt.hist(err_mlp, bins=30, histtype='step', label='MLP', color='green', linewidth=1.5, density=True)
    plt.hist(err_std, bins=30, histtype='step', label='Std SCN', color='blue', linewidth=1.5, density=True)
    # DWMP 使用填充颜色突出显示
    plt.hist(err_dwmp, bins=30, histtype='stepfilled', label='DWMP-SCN', color='red', alpha=0.3, density=True, edgecolor='red')
    
    plt.xlabel('Prediction Error (MW)')
    plt.ylabel('Probability Density')
    plt.title('(b) Error Distribution Comparison')
    plt.legend(loc='upper right', frameon=True)
    
    plt.tight_layout()
    plt.show()

    # --- Figure 2: CCPP 预测详情 + 局部放大 (含全模型) ---
    ...
    ax.plot(x, y_test_real[:limit], 'k-', label='Actual', linewidth=2, alpha=0.6)
    ax.plot(x, y_pred_mlp_real[:limit], 'g:', label='MLP', linewidth=1.5)
    ax.plot(x, y_pred_std_real[:limit], 'b--', label='Std SCN', linewidth=1.5)
    ax.plot(x, y_pred_dwmp_real[:limit], 'r-', label='DWMP-SCN', linewidth=2)
    ...
    # 放大区间
    z_start, z_end = 50, 75
    axins.plot(x[z_start:z_end], y_test_real[z_start:z_end], 'k-', linewidth=2, alpha=0.6)
    axins.plot(x[z_start:z_end], y_pred_mlp_real[z_start:z_end], 'g:', linewidth=1.5)
    axins.plot(x[z_start:z_end], y_pred_std_real[z_start:z_end], 'b--', linewidth=1.5)
    axins.plot(x[z_start:z_end], y_pred_dwmp_real[z_start:z_end], 'r-', linewidth=2)
